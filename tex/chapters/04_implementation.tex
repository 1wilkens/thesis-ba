...
\begin{itemize}
    ...
    \item{and optionally an empty initialized git repository (if the \shinline{-{}-git} command line option has been passed)}
\end{itemize}
The resulting application is already runnable via \mdinline{cargo run}\footnote{which is executable anywhere inside the project directory} and produces some output in \texttt{stdout}
