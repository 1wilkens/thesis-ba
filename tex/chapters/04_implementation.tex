...
All structures contain the expected data. Only the \shinline{cur} field in \shinline{dgraph} seems a little inconvenient. It needed to be added since Glib's \shinline{GHashTable} only support operations on all key-value-pairs via a function pointer with a single extra argument. Since the algorithm at on particular required access to the currently explored node's index as well as the distance and parent arrays the index needed to be stored in the struct itself.

While the additional field was a minor inconvenience other problematic aspects were the verbose access and missing typesafety of the used \shinline{GHashTable}s\fnote{The hash table implementation provided by GLib}. Since C is not typesafe by design and does not allow for true generic programming via type parameters nearly all generic code is written using \lstinline[style=c]{void*}. This leads to very verbose code because of the high amount of casts involved when accessing or storing values inside the data structures mentioned earlier.
...
//TODO: short description of the algorithm (add listing?)

All in all the experience was very poor compared to the other languages. Handling various corner cases with low to no real support from the compiler takes time which could be spent implementing actual functionality.
