...
\abstract{%
The first section of the third chapter describes the existing application the evaluation is based on. In addition the various stages of the development process of the programs to compare are illustrated.
}
...
The fifth and final stage consisted of modifying the existing benchmark to operate in parallel via threading and benchmarking the results for various configurations. While all the development and previous benchmarks were performed on a personal laptop the final benchmarks were taken on a cluster(??) of the DKRZ to gather relevant results in high concurrency situations.
